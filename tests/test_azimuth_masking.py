...
class TestBufferSymmetry:
    ...
    def test_buffer_symmetry(self, azimuth, buffer):
        ...
        assert len(result) == expected_size, (
            f"At azimuth {azimuth} with buffer {buffer}: " f"expected {expected_size} azimuths, got {len(result)}"
        )
    ...
    def test_turbine_always_included(self, azimuth):
        ...
        for buffer in [0, 1, 2, 5]:
            result = apply_azimuth_mask_for_test(azimuth, buffer, 360)
            assert azimuth in result, f"Turbine azimuth {azimuth} not in mask with buffer {buffer}"
...
class TestEdgeCases:
    ...
    def test_full_circle_coverage(self):
        ...
        assert len(result) == 360, f"Expected full coverage, got {len(result)} azimuths"
