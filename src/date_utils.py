...
def get_chunked_date_range(startdate, enddate, num_chunks, max_chunk_len=None, min_chunk_len=None):
    ...
    if max_chunk_len is not None or min_chunk_len is not None:
        ...
        if min_chunk_len is not None and np.any(np.array(array_lengths) < min_chunk_len):
        ...
        if max_chunk_len is not None and np.any(np.array(array_lengths) > max_chunk_len):
